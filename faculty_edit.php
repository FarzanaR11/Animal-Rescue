...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($name_err) && empty($course_err) && empty($project_id_err)&& empty($project_link_err)){
        ...
        $sql = "UPDATE students SET name=?, course=?, project_id=?, project_link=?, marks=? WHERE id=?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssii", $param_name, $param_course, $param_project_id, $param_project_link, $param_marks, $param_id);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: faculty_intro.php");
                exit();
            } else{
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($project_id_err)) ? 'has-error' : ''; ?>">
                            ...
                            <input type="text" name="project_id" class="form-control" value="<?php echo $project_id; ?>" readonly>
                            <span class="help-block"><?php echo $project_id_err;?></span>
                        </div>
                         <div class="form-group <?php echo (!empty($project_link_err)) ? 'has-error' : ''; ?>">
                            <label>Project link</label>
                            <input type="text" name="project_link" class="form-control" value="<?php echo $project_link; ?>" readonly>
                        ...
                        <a href="faculty_intro.php" class="btn btn-default">Cancel</a>
                    </form>
                </div>
            </div>        
        </div>
